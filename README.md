...
[![Version](https://img.shields.io/badge/version-1.1.1-blue.svg)](https://github.com/Kunal039/synapse-resilient-supply-chain/releases/tag/v1.1.1)
...
chains — a personal project exploring what "graduated autonomy" could look
like in practice. It's a real, running Node.js + browser app — not a mockup: a
synthetic multi-tier supply chain graph, a genuine forward-propagation "blast
radius" simulation, a scored Autonomy Gate, and a live control-tower dashboard
...
silently falls back to the template — the demo never shows an error.
...
2. **Click "Suez Canal Route Closure."** Watch it stream:
   ...
   **auto-executes** → the graph turns green and a toast confirms it.
   That's the routine 80% — Synapse just fixed it, no one got paged.
3. **Click "Single-Source Chip Fab Outage."** Same pipeline, but the score
   ...
   reasoning trail. Click **Approve & Execute**. This is the consequential
   20% — Synapse still did all the work, but a human made the final call.
...
5. **Point at the stats bar**: disruptions handled, auto-executed vs.
   escalated — the whole "graduated autonomy" idea in one glance.
